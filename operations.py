...
def translate_staff_position(env):
    ...
    while(len(env_lib.requested_coffee) != len(env_lib.requested_staff)):
        env_lib.requested_coffee.append(find_location_in_env(env,env_lib.requested_staff[listloop])) #translate from mapped name to location
        listloop += 1
...
def add_pos(row,col):
    ...
    env_lib.env[row][col] = 'CR' #robot location is now mapped here

def delete_pos(row,col):
    if env_lib.mapped_names[row][col] == 'CR':
        env_lib.env[row][col] = '0'
    else:
        env_lib.env[row][col] = env_lib.mapped_names[row][col] #mapped location where the robot once was
...
def open_door(row,col):
    if check_door_status(row,col) == 'CDW':
            env_lib.door_stat[row][col] = 'ODW' #door is now opened
...
def move_to(row,col):
    ...
    if check_pre_condition(row,col) == True and env_lib.env[row][col] != '-1':
        if 'DW' in env_lib.env[row][col] and check_door_status(row,col)  == 'CDW':
            ...
            add_pos(row,col)
            env_lib.path.append([row,col]) #keep track of where the robot is going
        else:
            delete_pos(cafe_robot.robot_position_row,cafe_robot.robot_position_col)	
            add_pos(row,col)
        ...
        for step in bfs_path:
            ...
            if step == current_goal:
                if env_lib.failed_state:
                    env_lib.requested_coffee.pop(0) #Reached Starting Position After Failure, Process Can Now Be Restarted
                    break
                else:
                    env_lib.requested_coffee.pop(0) #Reached Goal Position
